...
**Revised 2026-09-17 (later still) — two more shapes added, both found by `monitor-tests`'s exhaustiveness pass:**
- **Connection-string embedded passwords** (`scheme://user:password@host` — `DATABASE_URL=postgresql://...`, `mysql://`, `redis://`, `mongodb(+srv)://`, ...). This is *positional*, not field-name-based, so the generic `*_KEY=`/... rule can't catch it — and it's exactly the `kyo_team`/Supabase `DATABASE_URL` shape this feature's own motivation names. Only the password is redacted; scheme/user/host/port/database name stay visible so the mirrored line is still useful for oversight (e.g. "connected to `db.example.supabase.co:5432/postgres`" remains legible).
- **Opaque (non-JWT) Bearer tokens** — e.g. an agent building `curl -H "Authorization: Bearer <token>"` as a *tool call input*, not merely appearing in a result. A non-JWT bearer token has no distinctive value shape of its own (unlike `sk-`/`ghp_`/`xox…`/`eyJ…`), so this is keyed on the literal `Bearer ` prefix rather than the token's contents.
...
**Revised 2026-09-17 (later) — the exclusion above shipped with a lockout bug, now fixed.** Telegram private-chat ids equal the user's id, and that id is *identical across every bot* the user talks to. The exclusion matched on chat id alone, checked in the shared `handleTelegramMessage`/`handleSlashCommand` used by every agent's bot — so if the destination were ever the owner's own DM (a legitimate, plan-endorsed option: "the destination is just a chat id in config, so a DM destination remains possible without code changes"), the owner's private-chat id would collide with `monitor.chatId` on *every* agent's bot, and every message/command the owner sent to *every* agent would be silently dropped. The exclusion now applies **only** to a group/supergroup destination (`chatId < 0`) **and only** on the bot whose agent id matches `config.supervisor` (defence in depth — group ids are Telegram-wide unique and never collide with a user id, so `chatId < 0` alone is already sufficient). A private-chat destination now works with **no exclusion at all**: mirrored messages land in that DM alongside the owner's own normal conversation with whichever bot posts there, and every other agent's conversation with the owner is unaffected. Telegram's `createForumTopic` rejects private chats (Topics only exist in supergroups), so a DM destination is automatically covered by the existing "topics unavailable → warn once, fall back to posting without a thread" handling — no separate code path was needed for that.
...
10. **Revised 2026-09-17, tightened 2026-09-17 (later).** A synthetic turn with 100 tool calls yields a message count that scales with Telegram's rate limit and how much content fits per message, not with the raw event count (packing), and never a message per event; delivery for one destination chat is serialized (never more than one send in flight at a time across agents sharing it, so separate agents don't multiply 429s against the same limit); a destructive ⚠️ block is delivered in the very next send **for the whole destination chat**, not merely "the next message for its own agent" — if *other* agents have pending routine-only backlogs when the ⚠️ arrives, those must wait behind it too, with the one exception of a send already in flight at the moment the ⚠️ arrives (which cannot be preempted); the per-agent backlog is bounded (oldest routine content collapses into one counted summary line past a fixed cap, with critical content — destructive calls, inbound messages, `tgcc_send` origins, replies — never collapsed or dropped); and no unhandled 429 crashes or stalls the pipeline.
...
14. **Added 2026-09-17 (later).** A private-chat (DM) monitor destination never suppresses the owner's messages or commands to any agent, including the agent whose bot happens to physically post the mirror (the supervisor's) — a colleague DM, an agent DM, and `/status`/`/new`/etc. all behave exactly as they would with no `monitor` block at all when `monitor.chatId` is a private chat id, even (especially) when that id equals the owner's own.
